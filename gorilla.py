...
some value (such as the size of the explosions or the colour of the gorillas), you only have to change it in one
place."""
...
"""Here are a bunch of colors. Pygame uses a tuple of three integers to specify a colour. The integers are for the
amount of Red, Blue, and Green (in order) in the colour. This is known as an RGB value.
...
def makeSurfaceFromASCII(ascii, fgColor=(255,255,255), bgColor=(0,0,0)):
    ...
    The first and last line in ascii are ignored. Otherwise, any X in ascii marks a pixel with the foreground colour
    and any other letter marks a pixel of the background colour. The Surface object has a width of the widest line
...
def inputMode(prompt, screenSurf, x, y, fgcol, bgcol, maxlen=12, allowed=None, pos='left', cursor='_', cursorBlink=False):
    ...
    parameter) on the screenSurf surface at the x, y coordinates. The text is in the fgcol colour with a bgcol colour
...
def makeCityScape():
    ...
    screenSurf.fill(SKY_COLOR) # fill in the surface with the background sky colour
...
def collideWithNonColor(pixArr, surfObj, rect, color):
    ...
    if it has any pixels that are not the colour specified by the "colour" parameter. This function is used to detect
